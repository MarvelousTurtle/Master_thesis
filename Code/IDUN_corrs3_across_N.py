...
neurons = np.arange(3, 101, 1) # Iterable
binsize = 0.02
...
corrs3_data = np.empty((populations, int(neurons[run] * (neurons[run] - 1) * (neurons[run] - 2) / 6)))
corrs3_pair = np.empty((populations, int(neurons[run] * (neurons[run] - 1) * (neurons[run] - 2) / 6)))
corrs3_ind = np.empty((populations, int(neurons[run] * (neurons[run] - 1) * (neurons[run] - 2) / 6)))
for p in range(populations):
    ...
    corrs3_data[p] = corrs3(subpop_spike_trains, connected=True)
    corrs3_pair[p] = corrs3(samples_pair, connected=True)
    corrs3_ind[p] = corrs3(samples_ind, connected=True)

filename = f"concorrs3_{neurons[0]}-{neurons[-1]}neurons_new.pkl"
try:
    ...
    params_quality[f"N={neurons[run]}"] = {"h":h, "J":J , "neuron_indices":neuron_indices , "perturb_params":perturb_params, \
                                           "corrs3_data":corrs3_data, "corrs3_pair":corrs3_pair, "corrs3_ind":corrs3_ind}
    ...
    params_quality["Info"] = f"Used concatenated spike times from concatenated_spike_times_26471_bank0_495neurons_6sessions.npz; \
                             ...
                             Third order correlations from data and from the pairwise model (based on as many samples as in the data)\
